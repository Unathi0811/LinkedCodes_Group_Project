...
const Forecast = () => {
  ...
  const getTodayWeather = () => {
    ...
    return weather.list.find(
      (item) => new Date(item.dt * 1000).toLocaleDateString() === todayDate
    );
  };
  ...
  if (loading) {
    ...
    }}><ActivityIndicator size="large" color="#202A44" /></View>;
  }
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.todayWeatherCard}>
        ...
        <Text style={styles.todayDate}>
          {new Date(todayWeather.dt * 1000).toLocaleDateString("en-US", {
            weekday: "short",
          })}{" "}
          , {new Date(todayWeather.dt * 1000).toLocaleDateString()}
        </Text>
        <View style={{ flexDirection: "row", justifyContent: "center" }}>
          <Text style={{ fontSize: 50, fontWeight: "bold", color: "#202A44" }}>
            {todayWeather?.main?.temp || 0}
          </Text>
          <Text style={{ fontSize: 40, color: "#202A44" }}>°C</Text>
        </View>
        <View style={{ alignItems: "center" }}>
          <Text style={{ fontSize: 18, marginBottom: 10 }}>
            {todayWeather?.weather?.[0]?.description || "Loading..."}
          </Text>
        </View>
      </View>
            ...
            <View style={styles.Block}>
              <View style={styles.firstBlock}>
                <Text style={styles.todayInfoText}>
                  Pressure: <FontAwesome name="tachometer" size={12} />{" "}
                  {todayWeather.main.pressure} hPa
                </Text>
                <Text style={styles.todayInfoText}>
                  Humidity: <FontAwesome name="tint" size={12} />{" "}
                  {todayWeather.main.humidity}%
                </Text>
                <Text style={styles.todayInfoText}>
                  Wind Speed: <MaterialIcons name="air" size={12} />{" "}
                  {todayWeather.wind.speed} m/s
                </Text>
              </View>
              <View style={styles.secondBlock}>
                <Text style={styles.todayInfoText}>
                  Wind Direction: <MaterialIcons name="navigation" size={12} />{" "}
                  {todayWeather.wind.deg}°
                </Text>
                <Text style={styles.todayInfoText}>
                  Cloud Cover: <MaterialIcons name="cloud" size={12} />{" "}
                  {todayWeather.clouds.all}%
                </Text>
                <Text style={styles.todayInfoText}>
                  Rain: <MaterialIcons name="water" size={12} />{" "}
                  {todayWeather.rain ? todayWeather.rain["1h"] : "0"} mm
                </Text>
              </View>
            </View>
          </View>
                ...
                <View key={index} style={styles.weatherItem}>
                  ...
                  <View style={styles.weatherDetails}>
                    ...
                    >
                      ...
                      >
                        {" "}
                        <MaterialIcons name="thermostat" size={12} />{" "}
                        {item.main.temp}°C
                      </Text>
                    </View>
                    ...
                    >
                      ...
                      <View
                        style={{
                          width: "50%",
                          borderLeftWidth: 1,
                          borderLeftColor: "#EAF1FF",
                        }}
                      >
                        <Text style={styles.infoText}>
                          {" "}
                          Wind Direction:{" "}
                          <MaterialIcons name="navigation" size={12} />{" "}
                          {item.wind.deg}°
                        </Text>
                        <Text style={styles.infoText}>
                          {" "}
                          Cloud Cover: <MaterialIcons
                            name="cloud"
                            size={12}
                          />{" "}
                          {item.clouds.all} %
