...
import { SafeAreaView, ActivityIndicator, Modal, View, Text, TouchableOpacity, StyleSheet } from 'react-native';
import { auth, db } from "../../../firebase"; // Adjust path as necessary
import { addDoc, collection, onSnapshot, query, orderBy, where, getDoc, doc } from "firebase/firestore";
  ...
  const fetchUserData = async (userId, setUsername = false) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        setUserAvatars((prev) => ({
          ...prev,
          [userId]: userData.profileImage || "https://via.placeholder.com/60",
        }));
        if (setUsername) {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setMessages(snapshot.docs.map((doc) => {
        ...
        return {
          ...
          user: {
            _id: data.user._id,
            avatar: userAvatars[data.user._id] || "https://via.placeholder.com/60",
          },
        };
      }));
        ...
        <SafeAreaView style={styles.chatContainer}>
          <View style={styles.chatHeader}>
            <Text style={styles.chatTitle}>
              Chat with {chatUsername} {/* Display the username */}
            </Text>
